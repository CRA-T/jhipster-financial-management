...
describe('Decision Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call engagement query and add missing value', () => {
      ...
      jest.spyOn(engagementService, 'query').mockReturnValue(of(new HttpResponse({ body: engagementCollection })));
      const expectedCollection: IEngagement[] = [engagement, ...engagementCollection];
      jest.spyOn(engagementService, 'addEngagementToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ decision });
      comp.ngOnInit();

      expect(engagementService.query).toHaveBeenCalled();
      expect(engagementService.addEngagementToCollectionIfMissing).toHaveBeenCalledWith(engagementCollection, engagement);
    ...
    it('Should call AnnexDecision query and add missing value', () => {
      ...
      const expectedCollection: IAnnexDecision[] = [...additionalAnnexDecisions, ...annexDecisionCollection];
      ...
      expect(annexDecisionService.addAnnexDecisionToCollectionIfMissing).toHaveBeenCalledWith(
  ...
  describe('Compare relationships', () => {
    describe('compareEngagement', () => {
      it('Should forward to engagementService', () => {
        ...
        expect(engagementService.compareEngagement).toHaveBeenCalledWith(entity, entity2);
      });
    });
